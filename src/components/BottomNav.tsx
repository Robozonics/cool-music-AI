...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-20 bg-obsidian/90 backdrop-blur-2xl border-t border-white/5 z-30 px-3 flex justify-between items-center pb-safe">
      {tabs.map((tab) => {
        const Icon = tab.icon;
          ...
          >
            ...
            {tab.id === 'samples' && isActive ? (
              <div className="relative">
                ...
                <span className="absolute -top-1 -right-1 w-2 h-2 bg-acid-lime rounded-full shadow-[0_0_6px_#a3e635] animate-pulse" />
              </div>
            ) : (
